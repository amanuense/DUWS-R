//  _trg = ["Outpost Airbase",5,50,getPosWorld this,true] execvm "createzone.sqf";
...
_trg2 setTriggerStatements["this", format["[""%1"",thislist] execvm 'misc\enterlocation.sqf'",_place], ""];
...
if (_fortified) then {
	[_fortifiedspawn] execvm "support\createopfortified.sqf";
	sleep 2;
};


_handle = [_trigger, _size] execvm "support\createoppatrol.sqf";
waituntil {scriptdone _handle};

//only spawn one patrol on debug mode
if(debugmode) exitWith { diag_log format ["createzone.sqf debug mode"]; };
...
if (_size < 251) exitWith {
	...
	_handle = [_trigger, _size] execvm "support\createoppatrol.sqf";
	waituntil {scriptdone _handle};
	_handle = [_trigger, _size] execvm "support\createopteam.sqf";
};
...
if (_size <= 1000) exitWith {
	...
	_handle = [_trigger, _size] execvm "support\createopteam.sqf";
	waituntil {scriptdone _handle};
	_handle = [_trigger, _size] execvm "support\createopwpteam.sqf";
	waituntil {scriptdone _handle};
	_handle = [_trigger, _size] execvm "support\createopteam.sqf";
	waituntil {scriptdone _handle};
	["OPF_F","armored",_trigger,_size] execVM "misc\random_veh.sqf";
};      
if (_size <= 1500) exitWith {
	_vehcreate = ["OPF_F","armored",_trigger,_size] execVM "misc\random_veh.sqf";
...
if (_size <= 2000) exitWith {
	...
	_handle = [_trigger, _size] execvm "support\createoppatrol.sqf";
	waituntil {scriptdone _handle};    
	["OPF_F","air",_trigger,_size] execVM "misc\random_veh.sqf";
};        
