...
[] spawn {
	...
	while {true} do {
		sleep 30;
		//calculate a safer spawn position
		_safe_pos = [WARCOM_blu_hq_pos, 30, 100, 3, 0, 1, 0] call BIS_fnc_findSafePos;
		_group = [_safe_pos, WEST, WARCOM_blu_attack_wave_type,[],[],blufor_ai_skill] call BIS_fnc_spawnGroup;
		_TFname = [1] call compile preprocessFile "misc\random_name.sqf";
