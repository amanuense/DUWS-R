...
_unitPos = getPosWorld _attachedUnit;
//hint format["%1",_attachedUnit];

// Attack waves main

if !(WARCOM_zones_controled_by_OPFOR isEqualTo []) then { // make sure opfor is controlling at least 1 zones
	sleep 2;
	...
	// find a zone *** end 
	_randomZone = [_randomZone, 10, 50, 3, 0, 1, 0] call BIS_fnc_findSafePos;
